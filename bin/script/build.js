...
const {
  pathHelper, isIncludeInArray, isCorrectContent, isInstallPackage, copyDir, loadProjectConfig, writeFileFromString
} = require('./util/util')
const {
  error, info, done, warn, log
} = require('./util/logger')
...
module.exports = function buildModule(options) {
  ...
  const {
    distFolder, featureSrcFolder, activitySrcFolder, sourcemapFolder, projectRootFolder
  } = folder
  ...
  function compileModule(pageModule, cb) {
    ...
    rm(targetFolder, (err) => {
      ...
      webpack(webpackConfig({
        moduleName, API_ENV, NODE_ENV, CITY_ENV, sourcemapModuleName, analyzer
      }), (e, stats) => {
        spinner.stop()
        // webpack 打包出错
        if (e) {
